...
// Expanded list of genres (we'll keep this for diverse recommendations, but won't show it to the user)
const GENRES = [
...
export async function getTrackRecommendation() {
  ...
  while (attempts < maxAttempts) {
    ...
    const selectedGenres = getRandomGenres(5).join(',');
    const response = await fetch('https://api.spotify.com/v1/recommendations?' + new URLSearchParams({
      limit: 10, // Increased limit to have more options to choose from
      market: 'US',
